...
public final class UkcpServerChannel extends AbstractNioMessageChannel implements ServerChannel, Runnable {
  ...
  private final ReusableHashMap<SocketAddress, UkcpServerChildChannel> childChannelMap = new ReusableHashMap<>();
  ...
  private final ReusableHashMap<SocketAddress, CloseWaitKcp> closeWaitKcpMap = new ReusableHashMap<>();
  ...
  private void scheduleUpdate(long tsUpdate, long current) {
    if (scheduleUpdateLog.isDebugEnabled()) {
      scheduleUpdateLog.debug("schedule delay: " + (tsUpdate - current));
    }
